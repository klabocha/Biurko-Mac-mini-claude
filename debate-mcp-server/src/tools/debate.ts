import { z } from "zod";
import type { ServerConfig, DebateMode, DebateResult, LLMProvider } from "../types.js";
...
export const debateSchema = z.object({
  ...
  mode: z
    ...
    .default("debate")
    .describe("Tryb debaty: quick_poll (1 runda), debate (3 rundy), deep_dive (5+ rund)"),
  models: z
    .array(z.string())
    .optional()
    .describe("Lista nazw modeli do uzycia (domyslnie: wszystkie skonfigurowane)"),
});
...
export async function runDebateTool(
  ...
  // Run debate rounds
  const partialResult = await engine.runDebate(input.question, input.mode as DebateMode);
...
function formatDebateResult(result: DebateResult): string {
  ...
  parts.push(`Tryb: ${result.mode} | Modele: ${result.models.join(", ")} | Rundy: ${result.rounds.length}`);
  parts.push("");
